...
export function PreviewShell({ children, preview = true }: { children: React.ReactNode; preview?: boolean }) {
  return (
    <div className="k3d-preview">
      ...
      <div className="preview-notice"><span className="status-dot" /> {preview ? "Local design preview" : "Kitchen3D"} <span className="notice-detail">· {preview ? "No enquiries sent or appointments booked" : "Send a secure enquiry · visits are confirmed separately"}</span></div>
      ...
      <footer className="site-footer">
        ...
        <div className="container footer-bottom"><span>KITCHEN3D LTD</span><span>{preview ? "Internal preview · Not a live booking service" : "Visits by arrangement · enquire online or call Reza"}</span></div>
      </footer>
    </div>
  );
}
