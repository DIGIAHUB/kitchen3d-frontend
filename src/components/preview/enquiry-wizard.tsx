...
export function EnquiryWizard({ journey }: { journey: Journey }) {
  ...
  function reset() {
    setAnswers(initialAnswers); setSelectedServices([]); setFiles([]); setFileError(""); setStep(0); setComplete(false);
  ...
  return <section className="enquiry-section container">
    ...
    <div className="enquiry-layout">
      ...
      <div className="wizard-panel">
        <div className="form-preview-note"><strong>Try the journey with sample details.</strong> Nothing is sent or uploaded. Entries stay in this page’s memory, not browser storage.</div>
        {!complete ? <>
          ...
          <form onSubmit={next} method="post" autoComplete="off">
            <fieldset disabled={!today} className="wizard-fields">
              ...
              <div className="wizard-actions">{step > 0 ? <button type="button" className="button button-outline" onClick={() => setStep(current => current - 1)}>← Back</button> : <span />} {step === 3 ? <button type="button" className="button button-dark" onClick={() => { setAnswers(initialAnswers); setSelectedServices([]); setFiles([]); setFileError(""); setComplete(true); }}>Finish preview <span aria-hidden="true">→</span></button> : <button type="submit" className="button button-dark">{step === 2 ? "Review your details" : "Continue"} <span aria-hidden="true">→</span></button>}</div>
            </fieldset>
          </form>
        </> : <div className="preview-complete"><span className="complete-symbol" aria-hidden="true">✓</span><p className="eyebrow">LOCAL PREVIEW COMPLETE</p><h2 ref={headingRef} tabIndex={-1}>That’s the journey.</h2><p>No enquiry was sent and no appointment was booked. Your sample details and selected files have been cleared.</p><p>In the live version, this step will connect to the approved Wix enquiry and booking workflow.</p><div className="complete-actions"><button type="button" onClick={reset} className="button button-dark">Try again ↻</button><Link href="/" className="text-link">Back to the homepage →</Link></div></div>}
      </div>
    </div>
  </section>;
}
